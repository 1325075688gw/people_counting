...
from Origin_Point_Cloud.visual_elev import visual_point_cloud
...
class UartParseSDK():
    ...
    def mix_all_queue_th(cls):
        while not common.stop_flag:
            ...
            for evm in cls.evm_queue:
                ...
                point_list.extend(evm_point_list.tolist())
            ...
            common.point_cloud_show_queue.put(point_list)
    ...
    @classmethod
    def show_frame(cls,xmin,xmax,ymin,ymax,detection_range):
        """
        先聚类，然后再可视化
        :return: None
        """
        time.sleep(1)
        multiprocessing.Process(target=visual_point_cloud,args=(common.point_cloud_show_queue,xmin,xmax,ymin,ymax,detection_range,)).start()

def run_system():

    config=common.config
    common.zmax=multiprocessing.Value('d',float(config.get('radar_params','zmax'))).value
    common.xmin=multiprocessing.Value('d',0).value
    common.xmax=multiprocessing.Value('d',6).value
    common.ymax=multiprocessing.Value('d',2.5).value
    ...
    relative_pos=[0,0]
    direction=[0,1]
    tilt=float(config.get('radar_params','radar'+str(radar_index+1)+'_tilt'))
    height=float(config.get('radar_params','radar'+str(radar_index+1)+'_height'))
    ...
    UartParseSDK.show_frame(common.xmin,common.xmax,common.ymin,common.ymax,common.detection_range)
