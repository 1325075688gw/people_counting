...
class FourPlots(QWidget):
    ...
    def __init__(self,point_cloud_show_queue,xmin,xmax,ymin,ymax,detection_range):
        super().__init__()
    ...
    def update_point_cloud(self):
        #更新点云
        if self.point_cloud_show_queue.empty():
            self.origin_scatter.setData([])
        else:
            point_cloud=self.point_cloud_show_queue.get()
            if len(point_cloud)==0:
                self.origin_scatter.setData([])
            else:
                point_cloud=np.array(point_cloud)[:,1:]
...
def visual_point_cloud(point_cloud_show_queue,xmin,xmax,ymin,ymax,detection_range):
    qapp = QtWidgets.QApplication(sys.argv)
    app = FourPlots(point_cloud_show_queue,xmin,xmax,ymin,ymax,detection_range)
    app.show()
    qapp.exec_()