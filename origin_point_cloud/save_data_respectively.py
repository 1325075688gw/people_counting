...
class UartParseSDK():
            ...
            if cls.evm_count == 1:
                ...
                for evm in cls.evm_queue:
                    ...
                    if evm.evm_num == 1:
                        ...
                        temp_point_list[:, 1] = 12 - temp_point_list[:, 1]
                        temp_point_list[:,0]=0-temp_point_list[:,0]
                        point_list += temp_point_list.tolist()
                cls._frame_num += 1
    ...
    def show_frame(cls, show_flag):
        ...
        # 杨家辉过滤后，显示原始点云
        if show_flag == 0:
            multiprocessing.Process(target=point_cloud_show,args=(1200,600,common.queue_for_cluster_transfer,)).start()
        else:
            ...
            # 杨家辉过滤后，显示聚类结果
            if show_flag == 1:
                multiprocessing.Process(target=show_cluster,args=(1800,900,common.point_cloud_show_queue,common.cluster_show_queue,)).start()
            # 郭泽中可视化
            else:
...
if __name__ == "__main__":
    ...
    uartParseSDK1 = UartParseSDK(True, "COM3", "COM4", "./radar_parameters.cfg", 2.2, 10)
    uartParseSDK2 = UartParseSDK(True, "COM23", "COM24", "./radar_parameters.cfg", 2.2, 10)
    # UartParseSDK.init_parameters(run_mode, r"./data/"+file_name, frame_num, visual_module)
    UartParseSDK.init_parameters(0,r'./data/data_6_9,单人前后走,第6次',1000,1)

    # t=multiprocessing.Process(target=spy_locpos,args=(common.queue_for_cluster_transfer,common.loc_pos,common.cluster_show_queue,))
    # t.start()