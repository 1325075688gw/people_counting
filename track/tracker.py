...
from track.utils import is_at_edge_in,is_out_area
    ...
    def association(self):
        ...
        for i in row_ind:
            ...
            if j < len(self.clusters):
                ...
                distance2zero=np.linalg.norm(self.tracks[track_id].predict_location)
                if distance2zero<=5:
                    if distance[i][j] < self.min_accept_distance:
                        self.assignment[track_id] = j
                else:
                    if distance[i][j]<self.min_accept_distance*distance2zero/5:
                        self.assignment[track_id]=j
    ...
    def pre_association(self):
        ...
        for i in row_ind:
            ...
            if j < len(self.unused_clusters):
                track_id=track_ids[i]
                distance2zero=np.linalg.norm(self.pre_tracks[track_id].predict_location)
                if distance2zero<=5:
                    if distance[i][col_ind[i]] < self.min_accept_distance:
    ...
    def deal_with_unassigned_points(self):
        for i in range(len(self.unused_clusters)):
            if i not in self.pre_assignment.values():
                if self.frame>self.longest_pause_frames:
        ...
        for track_id in self.pre_tracks:
            ...
            if len(track.locations)==self.in_frames:
                ...
                if (self.in_frames-track.not_detected_times)/self.in_frames>self.in_rate:
                    print('转正!',track_id,track.not_detected_times)
