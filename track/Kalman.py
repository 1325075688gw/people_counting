...
from track.utils import is_out_area, is_at_edge_in,is_at_edge_out
...
class Multi_Kalman_Tracker():
    ...
    def predict(self):
        for track_id in self.tracks:
            ...
            start = max(0, int(len(track.u) - self.min_in_last_times / 10))
            track.s_ = np.matmul(self.transitionMatrix, track.s) + np.matmul(self.B, track.u[start:].mean(axis=0))
            track.P_ = np.matmul(np.matmul(self.transitionMatrix, track.P),
    ...
    def association(self):
        ...
        for i in range(len(self.tracks)):
            ...
            if j < len(self.clusters):
                ...
                distance2zero = np.linalg.norm(self.tracks[track_id].s_)
                if distance2zero <= 5:
                    if distance[i][j] < self.G:
                        self.d[track_id] = j
                else:
                    if distance[i][j] < self.G * distance2zero / 5:
                        self.d[track_id] = j
    ...
    def pre_association(self):
        ...
        for i in range(len(self.pre_tracks)):
            ...
            if j < len(self.unused_clusters):
                track_id = track_ids[i]
                distance2zero = np.linalg.norm(self.pre_tracks[track_id].s_)
                if distance2zero <= 5:
                    if distance[i][j] < self.G:
                        self.d[track_id] = j
                else:
    ...
    def update_assigned_pre_tracks(self):
        ...
        for j in range(len(self.unused_clusters)):
            if j in used_clusters:
                continue
            if self.frame > 200 and not is_at_edge_in(self.unused_clusters[j]):
                continue
    ...
    def nextFrame(self, clusters, heights, frame):
        ...
        if len(self.origin_clusters) > 150:
    ...
    def cal_distance(self):
        ...
        for track_id in self.tracks:
            ...
            row = np.array([])
            for j in range(len(self.clusters)):
                row = np.append(row, [np.linalg.norm(self.clusters[j] - track.s_) * 0.8 + 0.2 * (
                            track.height.height[-1] - self.heights[j])], axis=0)
            if distance.size == 0:
    ...
    def cal_pre_distance(self):
        ...
        for track_id in self.pre_tracks:
            ...
            for j in range(len(self.unused_clusters)):
                row = np.append(row, [
                    np.linalg.norm(np.array(self.unused_clusters[j]) - np.array(track.s_)) * 0.8 + 0.2 * (
                                track.height.height[-1] - self.unused_heights[j])], axis=0)
            if distance.size == 0:
                distance = np.array([row])
            else:
                distance = np.append(distance, [row], axis=0)
    ...
    def get_frame(self):
        if self.frame < 49 + self.M:
            return 49
        return self.frame - self.M

    def get_origin_cluster(self):
        if len(self.origin_clusters) < self.M + 1:
            return []
        else:
            return self.origin_clusters[-self.M - 1]
