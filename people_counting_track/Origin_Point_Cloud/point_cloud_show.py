...
from Origin_Point_Cloud.visual_point_cloud import visual_point_cloud
...
class UartParseSDK():
    ...
    @classmethod
    def show_frame(cls, xmin,xmax,ymax,detection_range):
        """
        先聚类，然后再可视化
        :return: None
        """
        time.sleep(1)
        multiprocessing.Process(target=visual_point_cloud,args=(common.point_cloud_show_queue,xmin,xmax,ymax,detection_range,)).start()
...
def run_system():
    ...
    common.ymax=multiprocessing.Value('d',7).value
    ...
    UartParseSDK.show_frame(common.xmin,common.xmax,common.ymax,common.detection_range)
