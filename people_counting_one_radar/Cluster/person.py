...
class Person:
    ...
    # 多雷达下训练结果
    # 3
    radar_training_result = [{person_features[j]:float(config.get('person_feature','radar'+str(i+1)+'_'+person_features[j])) for j in range(len(person_features))} for i in range(1)]
