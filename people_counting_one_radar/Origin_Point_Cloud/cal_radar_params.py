...
from Origin_Point_Cloud.coordinate_transfer.compute_radar_pose import run_one_radar
...
class UartParseSDK():
    ...
    @classmethod
    def init_parameters(cls, x,y,xd,yd,end_size):
        cls.x=x
        cls.y=y
        cls.xd=xd
        cls.yd=yd
        ...
        while True:
            if cls.current_size >= cls.end_size:
                ...
                for evm in range(len(cls.evm_queue)):
                    ...
                    file.flush()
                    file.close()
                print('正在计算雷达照射到的人的点云特征')
                try:
                    radar1_person_feature=run_one_radar(filepath)
