...
class Tracker():
    ...
    def get_unused_clusters(self):
        if len(self.assignment)==0:
            self.unused_heights=self.heights
            self.unused_clusters=self.clusters
        else:
            self.unused_clusters=[]
            self.unused_heights=[]
            for i in range(len(self.clusters)):
                if i not in self.assignment.values():
                    for j in self.assignment.values():
                        if np.linalg.norm(self.clusters[i]-self.clusters[j])>track_common.min_distance:
        ...
        #     if i not in self.assignment.values():
        #         for j in self.assignment.values():
        #             distance=np.linalg.norm(self.clusters[j]-self.clusters[i])
    ...
    def validate_tracks(self):
        ...
        for track_id in self.pre_tracks:
            ...
            if len(track.locations)>=self.in_frames:
                to_be_deleted.append(track_id)
                if (self.in_frames-track.not_detected_times)/self.in_frames>self.in_rate and track.get_move_range()>common.min_move_range:
