...
from utils import is_out_area,is_at_edge
...
class Multi_Kalman_Tracker():
    ...
    def update_assigned_pre_tracks(self):
        ...
        for j in range(len(self.unused_clusters)):
            if j in used_clusters:
                continue
            if self.frame>500 and not is_at_edge(self.unused_clusters[j]):
                continue
    ...
    def cal_pre_distance(self):
        ...
        for track_id in self.pre_tracks:
            ...
            row = np.array([])
            for j in range(len(self.unused_clusters)):
                row = np.append(row, [np.linalg.norm(np.array(self.unused_clusters[j]) - np.array(track.s_))*0.8+0.2*(track.height.height[-1]-self.unused_heights[j])], axis=0)
            if distance.size == 0:
                distance = np.array([row])
            else:
                distance = np.append(distance, [row], axis=0)
