...
def receive_data_th(uartParseSDK):
    global start_time
    start_time = time.time() / 1000
    while not common.stop_flag:
        data = uartParseSDK.data_port.read(uartParseSDK.bytes_num)
        uartParseSDK.bytes_data += data
        uartParseSDK.bytes_data = uartParseSDK.get_frame(uartParseSDK.bytes_data)

def receive_data_thread(uartParseSDK):
    """
    定义读取毫米波雷达采集数据的线程
    :return:接受数据线程
    """
    _receive_data_th = Process(target=receive_data_th,args=(uartParseSDK,))
    return _receive_data_th

def put_queue_thread(uartParseSDK, save_flag, save_path, second, end_size):
    """
    定义将毫米波雷达采集到数据放到队列中，供杨家辉调用【线程】
    :return: None
    """
    uartParseSDK.end_size = end_size
    _put_queue_th = Process(target=put_queue_th, args=(save_flag, save_path, second,))
    return _put_queue_th

def put_queue_th(uartParseSDK, save_flag, save_path, second):
        """
        ...
        end_time_frame = 0
        while not common.stop_flag:
            print("frame_num:{0}".format(uartParseSDK.frame_num))
            if uartParseSDK.frame_num < 70:
            ...
            t = time.time() * 1000
            t = int(round(t))
            temp["frame_num"] = uartParseSDK.frame_num
            temp["time_stamp"] = t
            temp["point_num"] = point_cloud_num
            temp["point_list"] = point_cloud_list
            frame_num = "frame_num_" + str(uartParseSDK.frame_num)
            ...
            for index, value in enumerate(cart_transfer):
                ...
                point_cloud_num += 1
            temp2 = dict()
            temp2["frame_num"] = uartParseSDK.frame_num
            temp2["time_stamp"] = t
            temp2["point_num"] = point_cloud_num
            temp2["point_list"] = point_cloud_list
            frame_num = "frame_num_" + str(uartParseSDK.frame_num)
            frame_dict_cart = {frame_num: temp2}
            if uartParseSDK.save_2_queue_flag == True:
                uartParseSDK.json_data_polar.update(frame_dict_polar)
                uartParseSDK.json_data_cart_transfer.update(frame_dict_cart)
            common.queue_for_cluster_transfer.put(temp2)
            uartParseSDK.current_size += 1

            if save_flag == -1:
                continue
            else:
                if save_flag == 0:
                    if uartParseSDK.current_size == uartParseSDK.end_size:
                    # if self.frame_num == frame_nums:
                        uartParseSDK.save_2_queue_flag = False
                        polar_copy = deepcopy(uartParseSDK.json_data_polar)
                        cart_transfer_copy = deepcopy(uartParseSDK.json_data_cart_transfer)
                        uartParseSDK.json_data_polar.clear()
                        uartParseSDK.json_data_cart_transfer.clear()
                        uartParseSDK.save_data_thread(polar_copy, cart_transfer_copy, save_path).start()
                elif save_flag == 1:
...
if __name__ == "__main__":
    ...
    receive_data_thread(uartParseSDK).start()
    put_queue_thread(uartParseSDK,-1, r"./data/data_5_31,1-8米，单人绕圈，第1次", 0 , 1000).start()
